...
int main( int, char** )
{
	...
	//Listing all devices avaliable
	Device devRealSensor;//this is the variable which will hold the device we'll use
	cout << "Avaliable devices (according to OPENNI):" << endl;
	openni::Array<openni::DeviceInfo> deviceList;
	openni::OpenNI::enumerateDevices(&deviceList);
	for (int i = 0; i < deviceList.getSize(); ++i)
	{
		cout<< i + 1 <<" "<< deviceList[i].getName() << " " << deviceList[i].getVendor() << " " << deviceList[i].getUri() << endl;
	}
	int chosen = 1;

	do{
		cout << "Choose device to open (1) [0 to exit]: " << endl;
